...
| `feature_upsell` | `true` | Включить upsell (комбо) |
| `feature_flashOffers` | `true` | Включить flash предложения |
| `flashDiscountPercent` | `99` | Размер скидки для flash товаров |
| `flashTimerDuration` | `120` | Длительность flash таймера (сек) |
| `upsellEnabled` | `true` | Включить upsell модальные окна |
| `upsellDelay` | `300` | Задержка показа upsell (мс) |
...
## 🏷️ Система префиксов ID товаров

В проекте используется уникальная система префиксов в ID товаров для определения их свойств и поведения:

### Условия в ID:
- **`H`** - Острое блюдо (красный бейдж 🌶️)
- **`Z`** - Запеченное блюдо (оранжевый бейдж 🔥)
- **`T`** - Теплое блюдо (зеленый бейдж 🔥)
- **`R2000`** - FLASH блюдо со скидкой 99% (⚡)
- **`X`** - Участвует в акции 1+1=3 (🎁)
- **`S`** - СЕТ, не участвует в скидках и флеш (📦)
- **`C`** - Шеф рекомендует (золотая корона 👑)
- **`W`** - Рекомендует к заказу (показывается в peeking popup ⭐)

### Построение КОМБО:
- **`Q`** - Закуски (для upsell шага 1 🥗)
- **`Y`** - Соусы (для upsell шага 2 🥫)
- **`D`** - Напитки (для upsell шага 3 🥤)

### Примеры ID товаров:
```
HC001  - Острая пицца "Шеф рекомендует"
ZT002  - Запеченная теплая паста
R2000_003 - FLASH пицца со скидкой 99%
SQ004  - Сет с закусками
WD005  - Рекомендуемый напиток
X006   - Товар в акции 1+1=3
```
...
if (isFlashProduct(product)) {
  // Показать flash таймер
  showFlashTimer();
}

if (isChefRecommended(product)) {
  // Показать корону шефа
  showChefBadge();
}

// Получить все префиксы товара
const prefixes = getProductPrefixes(product.id);
console.log('Префиксы:', prefixes);

// Получить товары для комбо
const appetizers = getComboProducts(products, 'appetizers'); // Q префикс
const sauces = getComboProducts(products, 'sauces');         // Y префикс
const drinks = getComboProducts(products, 'drinks');         // D префикс

// Проверить нужно ли показать upsell
if (isMainDish(product)) {
  // Запустить upsell поток
  startUpsellFlow();
}
```

### Настройка префиксов через Google Sheets
Добавьте в лист `settings` для кастомизации префиксов:
```
product_prefix_H_label     | Очень острое
product_prefix_H_color     | #ff0000
product_prefix_H_emoji     | 🔥🌶️

product_prefix_C_label     | Хит от шефа
product_prefix_C_color     | #gold
product_prefix_C_emoji     | 👨‍🍳👑
```
